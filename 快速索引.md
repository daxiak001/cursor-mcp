...
- 运行说明与脚本：
  ...
  - ./scripts/enable_mcp.ps1 / ./scripts/disable_mcp.ps1
  - ./scripts/affected_domains.cjs / ./scripts/generate_domain_smoke_tests.cjs / ./scripts/augment_tests.cjs / ./scripts/smoke_analyze.cjs / ./scripts/check_smoke.ps1 / ./scripts/interface_diff.cjs / ./scripts/contract_check.cjs / ./scripts/run_reports.ps1
  - npm run ci:all（预检/差异/域/冒烟/接口/契约/门禁/汇总）
  - npm run ci:diff:pattern（输出 reports/diff-test-pattern.txt）
  - npm run guard:experience（生成经验守护清单） / npm run guard:start（开启实时守护）
  - npm run guard:pre（开发前预防：生成 dev-precheck.md）
- 团队协作：
  - ./贡献指南.md / ./新成员上手.md
- PR与质量：
...
- 策略与门禁（S2）：
  ...
  - 新工件（CI）：reports/domain-smoke-created.txt（域冒烟占位生成计数）
  - 新工件（CI）：reports/smoke-analysis.{json,md}（域冒烟加权通过率）
  - 新工件（CI）：reports/smoke-gate.txt（加权冒烟门禁结果）、reports/smoke-top-failed.md（Top失败域）
  - 质量门禁配置：policy/quality-gates.json（smoke.weighted_pass_threshold, mode=warn|fail）
  - 强制理解确认：IR-031（执行前必须输出理解/方案/步骤/风险/确认点；团队模式由“小户”确认）
  - 快捷确认：输入“~1”视为“执行”同等指令
